...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  */
  ...
  const newOracle = config.oracle.alpacaV2Oracle02;

  const iMoneyMarketFactory = await IMoneyMarket__factory.connect(config.moneyMarket.moneyMarketDiamond, deployer);

  console.log(`> 🟢 setOracle : ${newOracle}`);

  const tx = await iMoneyMarketFactory.setOracle(newOracle);
