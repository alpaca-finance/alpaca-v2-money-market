...
contract LYF_ReinvestTest is LYF_BaseTest {
  ...
  function testCorrectness_WhenReinvestisCalled_ShouldConvertRewardTokenToLP() external {
    ...
    LibLYF01.LPConfig memory _lpConfig = viewFacet.lpConfigs(address(wethUsdcLPToken));
    ...
    assertEq(viewFacet.lpValues(address(wethUsdcLPToken)), 30 ether);
    ...
    assertEq(viewFacet.lpValues(address(wethUsdcLPToken)), 31.5 ether);
    assertEq(viewFacet.pendingRewards(address(wethUsdcLPToken)), 0);
  }
  ...
  function testCorrectness_WhenPendingRewardLessThanReinvestThreshold_ShouldSkipReinvest() external {
    ...
    assertGt(viewFacet.pendingRewards(address(wethUsdcLPToken)), 0);
  }
    ...
    external
  {
    ...
    assertEq(viewFacet.lpValues(address(wethUsdcLPToken)), _bobLpAmount);
    assertEq(viewFacet.lpShares(address(wethUsdcLPToken)), _bobLpAmount);
    ...
    // totalShare = 50 + 25 = 75
    assertEq(viewFacet.lpValues(address(wethUsdcLPToken)), 90 ether);
    assertEq(viewFacet.lpShares(address(wethUsdcLPToken)), 75 ether);
    assertEq(viewFacet.subAccountCollatAmount(_bobSubaccount, address(wethUsdcLPToken)), 50 ether);
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubaccount, address(wethUsdcLPToken)), 25 ether);
  }
    ...
    external
  {
    ...
    // totalLPValues = 75
    // totalShares = 75
    assertEq(viewFacet.lpValues(address(wethUsdcLPToken)), 75 ether);
    assertEq(viewFacet.lpShares(address(wethUsdcLPToken)), 75 ether);
    ...
    // totalShares = 65
    assertEq(viewFacet.lpValues(address(wethUsdcLPToken)), 73.666666666666666667 ether);
    assertEq(viewFacet.lpShares(address(wethUsdcLPToken)), 65 ether);
  }
