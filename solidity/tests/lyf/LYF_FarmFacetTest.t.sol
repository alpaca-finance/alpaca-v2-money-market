...
contract LYF_FarmFacetTest is LYF_BaseTest {
  ...
  function testCorrectness_WhenUserBorrowTokenFromMM_ShouldTransferTokenToUser() external {
    ...
    farmFacet.borrow(subAccount0, address(weth), _borrowAmount);
    vm.stopPrank();
    ...
    uint256 _debtAmount;
    (, _debtAmount) = farmFacet.getDebt(BOB, subAccount0, address(weth));
    assertEq(_debtAmount, _borrowAmount);
    // sanity check on subaccount1
    (, _debtAmount) = farmFacet.getDebt(BOB, subAccount1, address(weth));
  ...
  //   uint256 _borrowAmount = 10 ether;
  //   vm.startPrank(BOB);
  //   vm.expectRevert(abi.encodeWithSelector(ILYFFarmFacet.LYFFarmFacet_InvalidToken.selector, address(mockToken)));
  //   farmFacet.borrow(subAccount0, address(mockToken), _borrowAmount);
  //   vm.stopPrank();
  // }
  ...
  //   farmFacet.borrow(subAccount0, address(weth), _aliceBorrowAmount);
  //   vm.stopPrank();

  //   LibDoublyLinkedList.Node[] memory aliceDebtShares = farmFacet.getDebtShares(ALICE, subAccount0);
  ...
  //   farmFacet.borrow(subAccount0, address(usdc), _aliceBorrowAmount2);
  //   vm.stopPrank();

  //   aliceDebtShares = farmFacet.getDebtShares(ALICE, subAccount0);
  ...
  //   // list will be add at the front of linkList
  //   vm.expectRevert(abi.encodeWithSelector(ILYFFarmFacet.LYFFarmFacet_NotEnoughToken.selector, _aliceBorrowAmount * 2));
  //   farmFacet.borrow(subAccount0, address(weth), _aliceBorrowAmount * 2);
  //   vm.stopPrank();
  // }
  ...
  //   farmFacet.borrow(subAccount0, address(isolateToken), _bobIsloateBorrowAmount);
  //   vm.stopPrank();
  // }
  ...
  //   farmFacet.borrow(subAccount0, address(weth), _bobIsloateBorrowAmount);
  //   vm.stopPrank();
  // }
  ...
  //   uint256 _borrowingPowerUSDValue = farmFacet.getTotalBorrowingPower(ALICE, subAccount0);
  ...
  //   vm.startPrank(ALICE);
  //   farmFacet.borrow(subAccount0, address(weth), 2.025 ether);
  //   farmFacet.borrow(subAccount0, address(usdc), 2.025 ether);
  //   vm.stopPrank();

  //   (uint256 _borrowedUSDValue, ) = farmFacet.getTotalUsedBorrowedPower(ALICE, subAccount0);
  ...
  //   vm.startPrank(ALICE);
  //   weth.approve(lyfDiamond, _aliceCollatAmount);
  //   ibWeth.approve(lyfDiamond, _ibTokenCollatAmount);
  ...
  //   vm.prank(ALICE);
  //   farmFacet.borrow(subAccount0, address(weth), 8.1 ether);
  ...
  //   vm.prank(ALICE);
  //   weth.transfer(lyfDiamond, 50 ether);
  ...
  //   vm.prank(ALICE);
  //   farmFacet.borrow(subAccount0, address(weth), 12.15 ether);

  //   (uint256 _borrowedUSDValue, ) = farmFacet.getTotalUsedBorrowedPower(ALICE, subAccount0);
  //   assertEq(_borrowedUSDValue, 13.5 ether);
  // }

  // function testRevert_WhenUserBorrowMoreThanLimit_ShouldRevertfarmFacetExceedBorrowLimit() external {
  ...
  //   vm.expectRevert(abi.encodeWithSelector(ILYFFarmFacet.LYFFarmFacet_ExceedBorrowLimit.selector));
  //   farmFacet.borrow(subAccount0, address(weth), _borrowAmount);
  //   vm.stopPrank();
  // }
}
