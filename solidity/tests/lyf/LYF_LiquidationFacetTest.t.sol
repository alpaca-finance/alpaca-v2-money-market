...
contract LYF_LiquidationFacetTest is LYF_BaseTest {
  ...
  function testCorrectness_WhenPartialRepurchase_ShouldWork() external {
    ...
    assertEq(
      viewFacet.subAccountCollatAmount(_aliceSubAccount0, _collatToken),
      10 ether - _wethReceivedFromRepurchase // TODO: account for repurchase fee
    );
    (, uint256 _aliceUsdcDebtValue) = viewFacet.getDebt(ALICE, subAccount0, address(usdc), _lpToken);
  ...
  function testCorrectness_WhenRepurchaseMoreThanDebt_ShouldRepurchaseAllDebtOnThatToken() external {
    ...
    (, uint256 _aliceWethDebtValue) = viewFacet.getDebt(ALICE, subAccount0, address(weth), _lpToken);
  ...
  function testCorrectness_WhenSubAccountWentUnderWater_ShouldBeAbleToLiquidate() external {
    ...
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(weth)), 10 ether);
    ...
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(weth)), 0);
  ...
  function testCorrectness_WhenSubAccountWentUnderWaterWithIbCollat_ShouldBeAbleToLiquidateIbCollat() external {
    ...
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, _collatToken), 4 ether);
    ...
    // collateral is sold to repay
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(ibBtc)), 0);
    // leftover underlying token is add to collateral
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(btc)), 3.495 ether);
  ...
  function testCorrectness_WhenPartialLiquidateLP_ShouldWork() external {
    ...
    // check alice position
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, _lpToken), 25 ether, "alice remaining lp collat");
    assertEq(
      viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(weth)),
      15 ether,
      "alice remaining weth collat"
    );
    assertEq(
      viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(usdc)),
      1 ether,
  ...
  function testCorrectness_WhenLiquidateLPMoreThanCollateral_ShouldLiquidateAllLP() external {
    ...
    // check alice position
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, _lpToken), 0, "alice remaining lp collat");
    assertEq(
      viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(weth)),
      40 ether,
      "alice remaining weth collat"
  ...
  function testCorrectness_WhenPartialLiquidateLPThatAlsoUsedAsCollat_ShouldWork() external {
    ...
    // check alice position
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, _lpToken), 45 ether, "alice remaining lp collat");
    assertEq(
      viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(weth)),
      1 ether,
      "alice remaining weth collat"
  ...
  function testCorrectness_WhenLiquidateLPButReceiveTokensLessThanRepayAmount_ShouldWork() external {
    ...
    assertEq(viewFacet.subAccountCollatAmount(_aliceSubAccount0, address(usdc)), 0, "alice remaining usdc collat");
    (, uint256 _aliceWethDebtValue) = viewFacet.getDebt(ALICE, subAccount0, address(weth), _lpToken);
    assertEq(_aliceWethDebtValue, 0, "alice remaining weth debt");
    (, uint256 _aliceUsdcDebtValue) = viewFacet.getDebt(ALICE, subAccount0, address(usdc), _lpToken);
