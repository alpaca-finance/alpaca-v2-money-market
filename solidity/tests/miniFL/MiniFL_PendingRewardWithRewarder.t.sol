...
contract MiniFL_PendingRewardWithRewarderTest is MiniFL_BaseTest {
  ...
  function testCorrectness_WhenTimpast_RewarderPendingTokenShouldBeCorrectly() external {
    ...
    assertEq(rewarder1.pendingToken(dtokenPoolID, ALICE), 100 ether);
    assertEq(rewarder1.pendingToken(dtokenPoolID, BOB), 900 ether);

    assertEq(rewarder2.pendingToken(wethPoolID, ALICE), 10000 ether);
    assertEq(rewarder2.pendingToken(wethPoolID, BOB), 5000 ether);
    assertEq(rewarder2.pendingToken(dtokenPoolID, ALICE), 0 ether);
    assertEq(rewarder2.pendingToken(dtokenPoolID, BOB), 0 ether);
  }
    ...
    external
  {
    ...
    debtToken1.mint(address(rewarder1), 10 ether);
